...
#' @param ncond An integer specifying the number of matched groups of N = 1
#'   per condition to which subjects are to be assigned.
#'
#' @return A matrix with \code{nsub} rows and \code{ncond} columns, where each
#'   row represents a subject and each column represents a set of matched
#'   subjects.
#'
#' @export
extract_group_assignment = function(ga_output, nsub, ncond) {
  assignment_mat = matrix(data = ga_output@solution[1,],
                          nrow = nsub,
                          ncol = ncond)
